...
Trajectory::Trajectory(const rclcpp::Time& current_time,
...
void Trajectory::set_point_before_trajectory_msg(
  ...
  if (current_point.accelerations.empty() && !trajectory_msg_->points[0].accelerations.empty()) {
    state_before_traj_msg_.accelerations.resize(trajectory_msg_->points[0].accelerations.size(),
                                                0.0);
  }

  // Compute offsets due to wrapping joints
  wraparound_joint(state_before_traj_msg_.positions, trajectory_msg_->points[0].positions,
                   joints_angle_wraparound);
}

void wraparound_joint(std::vector<double>& current_position,
  ...
  for (size_t i = 0; i < joints_angle_wraparound.size(); i++) {
    if (joints_angle_wraparound[i]) {
      dist = shortest_angular_distance(current_position[i], next_position[i]);
...
bool Trajectory::sample(const rclcpp::Time& sample_time,
  ...
  if (sample_time < first_point_timestamp) {
    ...
    if (interpolation_method == interpolation_methods::InterpolationMethod::NONE) {
      ...
      deduce_from_derivatives(state_before_traj_msg_, first_point_in_msg,
                              state_before_traj_msg_.positions.size(),
                              (first_point_timestamp - time_before_traj_msg_).seconds());

      interpolate_between_points(time_before_traj_msg_, state_before_traj_msg_,
                                 first_point_timestamp, first_point_in_msg, sample_time,
                                 output_state);
    }
  ...
  for (size_t i = last_sample_idx_; i < last_idx; ++i) {
    ...
    if (sample_time >= t0 && sample_time < t1) {
      ...
      // Do interpolation
        ...
        // it changes points only if position and velocity do not exist, but their derivatives
        deduce_from_derivatives(point, next_point, state_before_traj_msg_.positions.size(),
                                (t1 - t0).seconds());
...
void Trajectory::deduce_from_derivatives(trajectory_msgs::msg::JointTrajectoryPoint& first_state,
...
rclcpp::Time Trajectory::time_from_start() const {
